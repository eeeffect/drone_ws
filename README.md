...
2. Встановіть Python залежності (обов'язкові):
```bash
pip install opencv-python numpy
```
3. *(Опціонально)* Встановіть залежності для нейромереж (**LoFTR** та **SuperPoint/LightGlue**). Рекомендовано для впевненого розпізнавання:
```bash
...
Для запуску ноди вкажіть шлях до файлу з вашим ортофотопланом (kartою) як параметр `map_path`. Також ви можете вибрати алгоритм розпізнавання за допомогою параметра `matcher_type` (можливі значення: `orb`, `loftr`, `superpoint`). За замовчуванням використовується `orb`.

### Запуск з класичним OpenCV ORB (швидко, тільки CPU):
```bash
ros2 run visual_localization localization_node --ros-args -p map_path:="/абсолютний/шлях/до/карти.png" -p matcher_type:="orb"
```

### Запуск з нейромережею LoFTR (висока точність, стійкість до зміни пір року/тепловізора, потребує GPU):
```bash
ros2 run visual_localization localization_node --ros-args -p map_path:="/абсолютний/шлях/до/карти.png" -p matcher_type:="loftr"
```

### Запуск з мережею SuperPoint + LightGlue (швидше ніж LoFTR, відмінна точність, потребує GPU):
```bash
ros2 run visual_localization localization_node --ros-args -p map_path:="/абсолютний/шлях/до/карти.png" -p matcher_type:="superpoint"
```
